// percepta-word/src/PerceptaWord.jsx
import { useEffect, useMemo, useRef, useState } from "react";

// percepta-word/BprSessionContract.js
...
var MODULE_CODE = "PERCEPTA_WORD";
var MODULE_VERSION = "1.7.0";
var BPR_DOMAIN = "PerceptaWord";
var DEFAULT_SCORING = {
...
function scoreAnswer(given, canonical, alternates = [], spec = DEFAULT_SCORING) {
  ...
  return { state: "needs_review", reason: "non_blank_mismatch", editDistance: dist, matched: canonical };
}
...
function answerKeyEntryFor(battery, order, practice = false) {
  if (practice) {
    const pItem = (battery.practiceTrials || []).find((i) => i.order === order);
    const canonical2 = (pItem?.originalName || "").replace(/^P?\d+[_-]+/, "").replace(/\.[^.]+$/, "").trim().toLowerCase();
    return { order, canonical: canonical2, alternates: [] };
  }
...
function PerceptaWord({ onComplete, onExit, candidate, session }) {
  ...
  const [timeLeft, setTimeLeft] = useState(battery?.timing?.totalSeconds ?? 240);
  ...
  const practiceTrials = useMemo(
    () => Array.isArray(battery.practiceTrials) ? [...battery.practiceTrials].sort((a, b) => a.order - b.order) : [],
    [battery]
  );
  const total = items.length;
  const totalSeconds = battery.timing?.totalSeconds ?? 240;
  const warnAt = battery.timing?.warnAtSeconds ?? 30;
  ...
  const REASON_RANK = { fuzzy: 0, phonetic: 1, non_blank_mismatch: 2 };
  const reviewItems = useMemo(
    () => items.filter((it) => verdicts[it.order]?.finalState === "needs_review").sort((a, b) => {
      const ra = REASON_RANK[verdicts[a.order]?.reason] ?? 99;
      const rb = REASON_RANK[verdicts[b.order]?.reason] ?? 99;
  ...
  function markAllRemainingWrong() {
    setVerdicts((prev) => {
      const next = { ...prev };
      for (const it of items) {
        if (next[it.order]?.finalState === "needs_review") {
          next[it.order] = { ...next[it.order], finalState: "incorrect", adminAdjudicated: true, adminMark: "incorrect" };
        }
      }
      return next;
    });
  ...
  function submitToNexus() {
    ...
    const trials_correct = responses.filter((r) => r.verdictState === "correct").length;
    const trials_incorrect = responses.filter((r) => r.verdictState === "incorrect").length;
    const trials_blank = responses.filter((r) => r.verdictState === "skipped").length;
    const trials_review_pending = responses.filter((r) => r.verdictState === "needs_review").length;
    const accuracy_pct = Math.round(trials_correct / trials_total * 100);
    ...
    const result = buildBprResult({
      ...
      summary_metrics: {
        // v1.4 cleanup: dropped redundant `score` (was a copy of accuracy_pct)
        // and `outcome` (this test is not pass/fail). Renamed
        // trials_passed → trials_correct, trials_failed → trials_incorrect
        // for clarity. trials_blank surfaced from raw_output.
        accuracy_pct,
        trials_total,
        trials_correct,
        trials_incorrect,
      ...
      raw_output: {
        ...
        trials_blank,
        trials_review_pending,
        // Spreadsheet-ready per-item results. One row per item; each row
        // carries enough fields to drive both candidate-facing reports
        // and the analytics database (verdictState is the canonical
        // right/wrong/blank/needs_review signal).
        responses,
    ...
    try {
      ...
      console.error("[PerceptaWord] validation failed", e);
    }
    onComplete(result);
  }
    ...
    /* @__PURE__ */ jsxs("main", { className: "flex-1 px-6 py-5 max-w-[1200px] w-full mx-auto", children: [
      ...
      phase === "practice" && /* @__PURE__ */ jsx(
        PracticePhase,
        {
          ...
          onExit,
          answerKeyFor: (o) => answerKeyEntryFor(
            battery,
            o,
            /*practice*/
            true
      ...
      phase === "admin-review" && /* @__PURE__ */ jsx(
        AdminReview,
        {
          ...
          verdicts,
          onMark: adminMark,
          onMarkAllRemainingWrong: markAllRemainingWrong,
  ...
  return /* @__PURE__ */ jsxs("header", { className: "px-6 py-3 border-b border-brand-gold/30 bg-brand-black/95 backdrop-blur flex items-center justify-between", children: [
    /* @__PURE__ */ jsxs("div", { children: [
      ...
      /* @__PURE__ */ jsxs("h1", { className: "text-base font-bold text-brand-cream tracking-wide", children: [
        "PerceptaWord\u2122 \u2014 Word Identification \xB7 Test #",
        battery.testNumber
      ] })
    ] }),
...
function Instructions({ battery, confirmedId, confirmedName, totalSeconds, onBegin, onBack, onExit }) {
  return /* @__PURE__ */ jsxs("div", { className: "max-w-2xl mx-auto py-8 space-y-6", children: [
    /* @__PURE__ */ jsxs("div", { children: [
      /* @__PURE__ */ jsx("h2", { className: "text-2xl font-bold text-brand-gold", children: "PerceptaWord\u2122 \u2014 Word Identification" }),
    ...
    /* @__PURE__ */ jsxs("div", { className: "rounded-xl border border-brand-gold/30 bg-brand-card p-5 space-y-4", children: [
      ...
      /* @__PURE__ */ jsxs("p", { className: "text-sm text-brand-cream", children: [
        "You will be presented with ",
        /* @__PURE__ */ jsxs("strong", { children: [
          battery.items.length,
          " partially obscured words"
        ] }),
        ". Identify each one using only the visible portions of the letters."
      ] }),
      /* @__PURE__ */ jsxs("ul", { className: "text-sm text-brand-cream list-disc pl-5 space-y-1.5", children: [
        ...
        /* @__PURE__ */ jsxs("li", { children: [
          "Click any number in the grid to jump to that word. Or use the keyboard: ",
        ...
        /* @__PURE__ */ jsx("li", { children: "Type your best guess in the answer box. You can revisit and change any answer." }),
        /* @__PURE__ */ jsxs("li", { children: [
          "The grid shows your progress only \u2014 answers are ",
          /* @__PURE__ */ jsx("strong", { children: "not graded" }),
          " while you take the test."
        ] }),
        /* @__PURE__ */ jsx("li", { children: "When time is up, you'll be prompted to grade. Anything left blank is marked as skipped." })
      ] })
    ] }),
...
function AdminReview({ items, reviewItems, reviewIdx, current, answers, verdicts, onMark, onMarkAllRemainingWrong, onSkip, onPrev, onFinish, answerKeyFor }) {
  ...
  const pendingByReason = items.reduce((acc, it) => {
    if (verdicts[it.order]?.finalState !== "needs_review") return acc;
    const r = verdicts[it.order]?.reason || "other";
    acc[r] = (acc[r] || 0) + 1;
    return acc;
  }, {});
  function confirmMarkAllWrong() {
    if (typeof window !== "undefined" && window.confirm) {
      const ok = window.confirm(
        `Mark all ${stillPending} remaining flagged item${stillPending === 1 ? "" : "s"} as INCORRECT?

This is recorded as an admin ruling on every item \u2014 not auto-fail. Use when the long tail is genuinely all wrong and you don't want to click through each one.`
      );
      if (!ok) return;
    }
    onMarkAllRemainingWrong?.();
  }
  ...
  return /* @__PURE__ */ jsxs("div", { className: "space-y-4 max-w-4xl mx-auto py-4", children: [
    /* @__PURE__ */ jsxs("div", { className: "rounded-xl border border-brand-gold/40 bg-brand-card p-4 space-y-2", children: [
      /* @__PURE__ */ jsxs("div", { className: "flex items-center justify-between gap-3 flex-wrap", children: [
        ...
        /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-1.5 text-[10px]", children: [
          pendingByReason.fuzzy > 0 && /* @__PURE__ */ jsxs("span", { className: "px-2 py-0.5 rounded bg-brand-medium/20 text-brand-medium border border-brand-medium/40 font-bold", children: [
            pendingByReason.fuzzy,
            " fuzzy"
          ] }),
          pendingByReason.phonetic > 0 && /* @__PURE__ */ jsxs("span", { className: "px-2 py-0.5 rounded bg-brand-medium/20 text-brand-medium border border-brand-medium/40 font-bold", children: [
            pendingByReason.phonetic,
            " phonetic"
          ] }),
          pendingByReason.non_blank_mismatch > 0 && /* @__PURE__ */ jsxs("span", { className: "px-2 py-0.5 rounded bg-brand-hard/20 text-brand-hard border border-brand-hard/40 font-bold", children: [
            pendingByReason.non_blank_mismatch,
            " wild miss"
          ] })
        ] })
      ] }),
      /* @__PURE__ */ jsxs("div", { className: "text-[11px] text-brand-dim flex items-center justify-between gap-3 flex-wrap", children: [
        /* @__PURE__ */ jsx("span", { children: "Finalize is locked until every flagged item is ruled." }),
        stillPending > 1 && /* @__PURE__ */ jsxs(
          "button",
          {
            onClick: confirmMarkAllWrong,
            className: "px-2.5 py-1 rounded-md bg-brand-hard/20 text-brand-hard border border-brand-hard/40 hover:bg-brand-hard/30 font-bold tracking-wide",
            title: "Mark all remaining flagged items as incorrect in one click. Useful when the long tail is genuinely all wrong.",
            children: [
              "\u2717 Mark remaining ",
              stillPending,
              " as incorrect"
    ...
    /* @__PURE__ */ jsxs("div", { className: "grid grid-cols-1 md:grid-cols-2 gap-4", children: [
      /* @__PURE__ */ jsxs("div", { className: "rounded-xl bg-brand-card border border-brand-gold/20 p-4 space-y-2", children: [
        /* @__PURE__ */ jsx("div", { className: "text-[11px] uppercase tracking-widest text-brand-muted", children: "Original word" }),
      ...
      return /* @__PURE__ */ jsxs("div", { className: "rounded-lg bg-brand-surface border border-brand-gold/15 p-3", children: [
        ...
        /* @__PURE__ */ jsxs("div", { className: "grid grid-cols-1 sm:grid-cols-3 gap-3 items-stretch", children: [
          /* @__PURE__ */ jsxs("div", { className: "space-y-1", children: [
            /* @__PURE__ */ jsx("div", { className: "text-[10px] uppercase tracking-widest text-brand-muted", children: "Actual word" }),
...
function PracticePhase({ practiceTrials, scoringSpec, onDone, onExit, answerKeyFor }) {
  const [activeIdx, setActiveIdx] = useState(0);
  const [practiceState, setPracticeState] = useState({});
  const inputRef = useRef(null);
  const total = practiceTrials.length;
  const activeItem = practiceTrials[activeIdx];
  const activeState = practiceState[activeItem?.order] ?? { text: "", viewed: false, viewCount: 0 };
  useEffect(() => {
    if (!activeItem) return;
    setPracticeState((prev) => {
      const cur = prev[activeItem.order] ?? { text: "", viewed: false, viewCount: 0 };
      return { ...prev, [activeItem.order]: { ...cur, viewed: true, viewCount: (cur.viewCount ?? 0) + 1 } };
    });
    setTimeout(() => inputRef.current?.focus(), 30);
  }, [activeIdx]);
  useEffect(() => {
  ...
  function onAnswerChange(text) {
    if (!activeItem) return;
    setPracticeState((prev) => {
      const cur = prev[activeItem.order] ?? { text: "", viewed: true, viewCount: 1 };
      return { ...prev, [activeItem.order]: { ...cur, text, verdict: void 0 } };
    });
  }
  function checkAnswer() {
    if (!activeItem) return;
    const cur = practiceState[activeItem.order] ?? { text: "" };
    const text = (cur.text ?? "").trim();
    const key = answerKeyFor(activeItem.order);
    if (!text) {
      setPracticeState((prev) => ({
        ...prev,
        [activeItem.order]: { ...cur, verdict: "skipped", canonical: key.canonical }
      }));
    } else {
      const v = scoreAnswer(text, key.canonical, key.alternates, scoringSpec);
      setPracticeState((prev) => ({
        ...prev,
        [activeItem.order]: { ...cur, verdict: v.state, canonical: key.canonical, reason: v.reason }
      }));
    }
    setTimeout(() => {
  ...
  if (!activeItem) {
    return /* @__PURE__ */ jsxs("div", { className: "text-center py-12 text-brand-muted", children: [
      /* @__PURE__ */ jsx("p", { children: "No practice items available." }),
      /* @__PURE__ */ jsx("button", { onClick: onDone, className: "mt-4 px-5 py-2 rounded-md bg-brand-gold text-black text-sm font-semibold", children: "Begin formal test \u2192" })
    ] });
  }
  const verdict = activeState.verdict;
  const verdictTone = verdict === "correct" ? "border-brand-easy bg-brand-easy/10" : verdict === "incorrect" ? "border-brand-hard bg-brand-hard/10" : verdict === "needs_review" ? "border-brand-medium bg-brand-medium/10" : verdict === "skipped" ? "border-brand-medium/40 bg-brand-medium/5" : "";
  const allDone = practiceTrials.every((t) => practiceState[t.order]?.verdict);
  return /* @__PURE__ */ jsxs("div", { className: "space-y-3", children: [
    /* @__PURE__ */ jsxs("div", { className: "rounded-xl border-2 border-brand-gold/60 bg-brand-card p-3 flex items-center justify-between gap-3", children: [
      /* @__PURE__ */ jsxs("div", { children: [
        /* @__PURE__ */ jsx("div", { className: "text-[10px] font-bold uppercase tracking-widest text-brand-gold", children: "Practice \u2014 the real test hasn't started" }),
        /* @__PURE__ */ jsx("div", { className: "text-xs text-brand-cream mt-0.5", children: `Try the interface. No timer here. After "I got it" you'll start the 4-minute timed test.` })
      ] }),
      /* @__PURE__ */ jsx(
        "button",
        {
          onClick: onDone,
          className: "px-5 py-2.5 rounded-md bg-brand-gold text-black text-sm font-bold hover:bg-brand-gold-light whitespace-nowrap",
          children: allDone ? "\u2713 I got it \u2014 Begin formal test \u2192" : "I got it \u2014 Begin formal test \u2192"
        }
      )
    ] }),
    /* @__PURE__ */ jsxs("div", { className: `rounded-2xl border-2 ${verdictTone || "border-brand-gold/30"} bg-[#f4f1ec] p-6`, children: [
      /* @__PURE__ */ jsxs("div", { className: "flex items-center justify-between mb-4", children: [
        /* @__PURE__ */ jsxs("div", { children: [
          /* @__PURE__ */ jsxs("div", { className: "text-[10pt] font-bold tracking-widest text-[#1a1a1a]", children: [
            "PRACTICE ",
        ...
        /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-2", children: [
          /* @__PURE__ */ jsx(
            "button",
            {
              ...
              className: "px-3 py-1.5 rounded bg-white border border-[#aaa] text-xs font-semibold disabled:opacity-30",
              children: "\u2190 Prev"
            }
          ),
          /* @__PURE__ */ jsx(
      ...
      /* @__PURE__ */ jsx("div", { className: "flex items-center justify-center bg-white rounded p-4 min-h-[2in] mb-4 border border-[#ddd]", children: activeItem.pngDataUrl ? /* @__PURE__ */ jsx(
        "img",
        {
          src: activeItem.pngDataUrl,
          alt: `Practice ${activeIdx + 1}`,
          style: { maxWidth: "100%", maxHeight: "1.6in", objectFit: "contain" }
        }
      ) : /* @__PURE__ */ jsx("span", { className: "text-xs text-[#999]", children: "No image" }) }),
      /* @__PURE__ */ jsxs("div", { className: "space-y-2", children: [
        /* @__PURE__ */ jsx("label", { className: "block text-[9pt] font-bold uppercase tracking-widest text-[#666]", children: "Your answer" }),
        /* @__PURE__ */ jsx(
          "input",
        ...
        verdict && /* @__PURE__ */ jsxs("div", { className: `text-center text-sm font-bold py-2 ${verdict === "correct" ? "text-brand-easy" : verdict === "incorrect" ? "text-brand-hard" : verdict === "skipped" ? "text-brand-dim" : "text-brand-medium"}`, children: [
          verdict === "correct" && "\u2713 Correct!",
          verdict === "incorrect" && /* @__PURE__ */ jsxs(Fragment, { children: [
            "\u2717 The word was ",
            /* @__PURE__ */ jsx("span", { className: "font-mono uppercase", children: activeState.canonical })
          ] }),
          verdict === "skipped" && /* @__PURE__ */ jsxs(Fragment, { children: [
            "\u2014 Skipped. The word was ",
            /* @__PURE__ */ jsx("span", { className: "font-mono uppercase", children: activeState.canonical })
          ] }),
          verdict === "needs_review" && /* @__PURE__ */ jsxs(Fragment, { children: [
            "\u2248 Close \u2014 the word was ",
            /* @__PURE__ */ jsx("span", { className: "font-mono uppercase", children: activeState.canonical })
          ] })
        ] }),
        !verdict && /* @__PURE__ */ jsx("p", { className: "text-[10px] text-[#666] text-center italic", children: "Press Enter to check your answer" })
      ] })
    ] }),
    /* @__PURE__ */ jsxs("div", { className: "rounded-xl bg-brand-card border border-brand-gold/20 p-3", children: [
      /* @__PURE__ */ jsx("div", { className: "text-[11px] uppercase tracking-widest text-brand-muted mb-2", children: "Practice items \xB7 click any to jump \xB7 arrows + Enter work too" }),
      /* @__PURE__ */ jsx("div", { className: "grid grid-cols-10 gap-2", children: practiceTrials.map((p, i) => {
        const v = practiceState[p.order]?.verdict;
        const isActive = i === activeIdx;
        const cls = v === "correct" ? "bg-brand-easy text-black border-2 border-brand-easy" : v === "incorrect" ? "bg-brand-hard text-white border-2 border-brand-hard" : v === "needs_review" ? "bg-brand-medium text-black border-2 border-brand-medium" : v === "skipped" ? "bg-brand-raised text-brand-muted border-2 border-brand-medium/50" : practiceState[p.order]?.viewed ? "bg-brand-raised text-brand-cream border-2 border-brand-medium/50" : "bg-brand-raised text-brand-muted border-2 border-transparent";
        return /* @__PURE__ */ jsxs(
          "button",
          {
            onClick: () => setActiveIdx(i),
            title: `Practice ${i + 1}: ${p.numberedName}`,
            className: [
              "h-9 rounded text-xs font-mono tabular-nums flex items-center justify-center transition-colors",
              cls,
              isActive ? "ring-4 ring-brand-medium ring-offset-2 ring-offset-brand-card" : ""
    ...
    /* @__PURE__ */ jsxs("div", { className: "rounded-lg bg-brand-raised border border-brand-gold/15 px-3 py-2 text-[10px] text-brand-muted flex flex-wrap items-center gap-3", children: [
      /* @__PURE__ */ jsx("span", { className: "font-bold text-brand-cream", children: "Keyboard:" }),
      /* @__PURE__ */ jsxs("span", { children: [
        /* @__PURE__ */ jsx("kbd", { className: "px-1.5 py-0.5 rounded bg-brand-card text-brand-cream font-mono text-[9px]", children: "\u2190" }),
        " ",
        /* @__PURE__ */ jsx("kbd", { className: "px-1.5 py-0.5 rounded bg-brand-card text-brand-cream font-mono text-[9px]", children: "\u2192" }),
        " navigate"
      ] }),
      /* @__PURE__ */ jsxs("span", { children: [
        /* @__PURE__ */ jsx("kbd", { className: "px-1.5 py-0.5 rounded bg-brand-card text-brand-cream font-mono text-[9px]", children: "Enter" }),
        " check answer + advance"
      ] }),
      /* @__PURE__ */ jsx("span", { children: "Type letters to answer \xB7 empty + Enter = skip" }),
      onExit && /* @__PURE__ */ jsx("button", { onClick: onExit, className: "ml-auto text-brand-dim hover:text-brand-cream", children: "Exit" })
    ] })
  ] });
}
function NoBatteryScreen({ onExit }) {
  return /* @__PURE__ */ jsx("div", { className: "min-h-screen bg-brand-black text-brand-cream flex items-center justify-center p-8", children: /* @__PURE__ */ jsxs("div", { className: "max-w-md text-center space-y-3", children: [
    /* @__PURE__ */ jsx("h1", { className: "text-xl font-bold text-brand-gold", children: "PerceptaWord\u2122" }),
    ...
    /* @__PURE__ */ jsxs("p", { className: "text-xs text-brand-dim", children: [
      `In production, the battery is baked into this module by PerceptaWord Studio's "Export as NEXUS module" tool. In the dev harness, set`,
